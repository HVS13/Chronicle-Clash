(() => {
  ...
  const cloneContentRoot = (doc = document, options = {}) => {
    ...
    clone.querySelectorAll('.cc-print-actions').forEach((node) => node.remove());
...
<html lang="en">
  ...
  const setMenuState = (menu, isOpen) => {
    ...
    const toggle = menu.querySelector('[data-cc-export-toggle]');
    const dropdown = menu.querySelector('.cc-export-dropdown');
    if (toggle) toggle.setAttribute('aria-expanded', isOpen ? 'true' : 'false');
    if (dropdown) dropdown.setAttribute('aria-hidden', isOpen ? 'false' : 'true');
  };

  const closeMenus = () => {
    document.querySelectorAll('[data-cc-export-menu].is-open').forEach((menu) => {
      setMenuState(menu, false);
    });
  };

  const bindMenus = () => {
    const menus = Array.from(document.querySelectorAll('[data-cc-export-menu]'));
    ...
    menus.forEach((menu) => {
      ...
      dropdown.addEventListener('click', (event) => {
        const item = event.target.closest('.cc-export-item[data-action]');
        if (!item) return;
        closeMenus();
      });
    });
    ...
    if (!document.body.dataset.uaExportMenuBound) {
      ...
      document.addEventListener('click', (event) => {
        if (!event.target.closest('[data-cc-export-menu]')) closeMenus();
      });
  ...
  const bindExportActions = () => {
    const buttons = Array.from(document.querySelectorAll('.cc-export-item[data-action]'));
