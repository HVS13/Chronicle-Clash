# Monkey D. Luffy (Pre-Timeskip)

<div class="character-header">
  <div class="character-image">
    <img src="../../assets/characters/monkey-d-luffy-pre-timeskip.png" alt="Monkey D. Luffy (Pre-Timeskip) portrait" loading="lazy" decoding="async" />
  </div>
  <div class="character-details">
    <div class="character-meta">
      <p><strong>Name:</strong> Monkey D. Luffy</p>
      <p><strong>Version:</strong> Pre-Timeskip</p>
      <p><strong>Origin:</strong> One Piece</p>
      <p><strong>Roles:</strong> <a class="cc-role-link" data-role="role-bruiser" href="#">Bruiser</a>, <a class="cc-role-link" data-role="role-striker" href="#">Striker</a></p>
      <p><strong>Difficulty:</strong> Medium</p>
      <p><strong>Gameplan:</strong> Lead with Gear Transformation: Gear 2nd for tempo and fast clashes, Gear 3rd for heavy hits. Plan around Deflate after Gear 3rd and use Jet Gatling as your high-pressure finisher when you are ahead.</p>
    </div>
  </div>
</div>

## Innates
<div class="card-block">
  <p class="card-block__title">Gomu Gomu no Mi</p>
  <p><a class="cc-keyword-link" data-keyword="keyword-resist" href="#">Resist</a> 5 (non-Haki Pierce, non-Haki Blunt).</p>
  <p><a class="cc-keyword-link" data-keyword="keyword-immune" href="#">Immune</a> (Electric).</p>
</div>

## Status Effects

<div class="cc-entry cc-entry--compact status-entry">
  <p class="cc-entry__title">Gear 2nd</p>
  <p class="cc-entry__meta"><span class="cc-pill">Type</span> Unique.</p>
  <p class="cc-entry__meta"><span class="cc-pill">Max Stack</span> 1.</p>
  <p class="cc-entry__meta"><span class="cc-pill">On Gain</span> Gain 1 <a class="cc-status-link" data-status="status-strength" href="#">Strength</a>. Gain 1 <a class="cc-status-link" data-status="status-haste" href="#">Haste</a>.</p>
  <p class="cc-entry__meta"><span class="cc-pill">Turn End</span> Expires.</p>
</div>

<div class="cc-entry cc-entry--compact status-entry">
  <p class="cc-entry__title">Gear 3rd</p>
  <p class="cc-entry__meta"><span class="cc-pill">Type</span> Unique.</p>
  <p class="cc-entry__meta"><span class="cc-pill">Max Stack</span> 1.</p>
  <p class="cc-entry__meta"><span class="cc-pill">On Gain</span> Gain 3 <a class="cc-status-link" data-status="status-strength" href="#">Strength</a>. Gain 1 <a class="cc-status-link" data-status="status-slow" href="#">Slow</a>.</p>
  <p class="cc-entry__meta"><span class="cc-pill">Turn End</span> Expires.</p>
  <p class="cc-entry__meta"><span class="cc-pill">When this expires</span> At your next Turn Start, gain Deflate.</p>
</div>

<div class="cc-entry cc-entry--compact status-entry">
  <p class="cc-entry__title">Deflate</p>
  <p class="cc-entry__meta"><span class="cc-pill">Type</span> Unique.</p>
  <p class="cc-entry__meta"><span class="cc-pill">Max Stack</span> 1.</p>
  <p class="cc-entry__meta"><span class="cc-pill">Effect</span> Cannot play any card from this character.</p>
  <p class="cc-entry__meta"><span class="cc-pill">Turn End</span> Reduce Stack by 1.</p>
</div>

## Cards

### Card 1: Strike
<div class="card-block">
  <div class="card-block__meta">
    <span><strong>Cost:</strong> 1 Energy</span>
    <span><strong>Power:</strong> 10-14</span>
    <span><strong>Type:</strong> Basic, Attack, Physical, Melee, Blunt</span>
    <span><strong>Target:</strong> 1 Enemy</span>
    <span><strong>Speed:</strong> Normal</span>
  </div>
  <p class="card-block__heading">Effect</p>
  <p>Deal Power damage.</p>
</div>

### Card 2: Defend
<div class="card-block">
  <div class="card-block__meta">
    <span><strong>Cost:</strong> 1 Energy</span>
    <span><strong>Power:</strong> 10-14</span>
    <span><strong>Type:</strong> Basic, Defense, Physical, Melee</span>
    <span><strong>Target:</strong> 1 Ally</span>
    <span><strong>Speed:</strong> Normal</span>
  </div>
  <p class="card-block__heading">Effect</p>
  <p>Gain Power Shield.</p>
</div>

### Card 3-1: Gomu Gomu no Pistol
<div class="card-block">
  <div class="card-block__meta">
    <span><strong>Cost:</strong> 1 Energy</span>
    <span><strong>Power:</strong> 12-18</span>
    <span><strong>Type:</strong> Technique, Attack, Physical, Melee, Blunt</span>
    <span><strong>Target:</strong> 1 Enemy</span>
    <span><strong>Speed:</strong> Normal</span>
  </div>
  <p class="card-block__heading">Effect</p>
  <p>Deal Power damage.</p>
  <p>If this character has Gear 2nd: This card becomes Gomu Gomu no Jet Pistol.</p>
  <p>If this character has Gear 3rd: This card becomes Gomu Gomu no Gigant Pistol.</p>
</div>

### Card 3-2: Gomu Gomu no Jet Pistol
<div class="card-block">
  <div class="card-block__meta">
    <span><strong>Cost:</strong> 1 Energy</span>
    <span><strong>Power:</strong> 12-18</span>
    <span><strong>Type:</strong> Technique, Attack, Physical, Melee, Blunt</span>
    <span><strong>Target:</strong> 1 Enemy</span>
    <span><strong>Speed:</strong> Fast</span>
  </div>
  <p class="card-block__heading">Effect</p>
  <p>Deal Power damage.</p>
</div>

### Card 3-3: Gomu Gomu no Gigant Pistol
<div class="card-block">
  <div class="card-block__meta">
    <span><strong>Cost:</strong> 1 Energy</span>
    <span><strong>Power:</strong> 12-18</span>
    <span><strong>Type:</strong> Technique, Attack, Physical, Melee, Blunt</span>
    <span><strong>Target:</strong> 1 Enemy</span>
    <span><strong>Speed:</strong> Slow</span>
  </div>
  <p class="card-block__heading">Effect</p>
  <p>Deal Power damage.</p>
</div>

### Card 4-1: Gomu Gomu no Bazooka
<div class="card-block">
  <div class="card-block__meta">
    <span><strong>Cost:</strong> 2 Energy</span>
    <span><strong>Power:</strong> 20-30</span>
    <span><strong>Type:</strong> Technique, Attack, Physical, Melee, Blunt</span>
    <span><strong>Target:</strong> 1 Enemy</span>
    <span><strong>Speed:</strong> Normal</span>
  </div>
  <p class="card-block__heading">Effect</p>
  <p>Deal Power damage.</p>
  <p>If this character has Gear 2nd: This card becomes Gomu Gomu no Jet Bazooka.</p>
  <p>If this character has Gear 3rd: This card becomes Gomu Gomu no Gigant Bazooka.</p>
</div>

### Card 4-2: Gomu Gomu no Jet Bazooka
<div class="card-block">
  <div class="card-block__meta">
    <span><strong>Cost:</strong> 2 Energy</span>
    <span><strong>Power:</strong> 20-30</span>
    <span><strong>Type:</strong> Technique, Attack, Physical, Melee, Blunt</span>
    <span><strong>Target:</strong> 1 Enemy</span>
    <span><strong>Speed:</strong> Fast</span>
  </div>
  <p class="card-block__heading">Effect</p>
  <p>Deal Power damage.</p>
</div>

### Card 4-3: Gomu Gomu no Gigant Bazooka
<div class="card-block">
  <div class="card-block__meta">
    <span><strong>Cost:</strong> 2 Energy</span>
    <span><strong>Power:</strong> 20-30</span>
    <span><strong>Type:</strong> Technique, Attack, Physical, Melee, Blunt</span>
    <span><strong>Target:</strong> 1 Enemy</span>
    <span><strong>Speed:</strong> Slow</span>
  </div>
  <p class="card-block__heading">Effect</p>
  <p>Deal Power damage.</p>
</div>

### Card 5: Gear Transformation
<div class="card-block">
  <div class="card-block__meta">
    <span><strong>Cost:</strong> 1 Energy</span>
    <span><strong>Power:</strong> -</span>
    <span><strong>Type:</strong> Technique, Special, Physical, Melee, Transformation</span>
    <span><strong>Target:</strong> Self</span>
    <span><strong>Speed:</strong> Normal</span>
  </div>
  <p class="card-block__heading">Effect</p>
  <p>Cannot be used if this character has Gear 2nd or Gear 3rd.</p>
  <p>Choose 1:</p>
  <p>Gain 1 Gear 2nd.</p>
  <p>Gain 1 Gear 3rd.</p>
  <p><a class="cc-keyword-link" data-keyword="keyword-retain" href="#">Retain</a>.</p>
</div>

### Ultimate-1: Gomu Gomu no Jet Gatling
<div class="card-block">
  <div class="card-block__meta">
    <span><strong>Cost:</strong> 30 Ultimate Meter + X Energy</span>
    <span><strong>Power:</strong> 24-36 + 8-12 times X</span>
    <span><strong>Type:</strong> Ultimate, Attack, Physical, Melee, Splash, Blunt, Multihit, Debuff</span>
    <span><strong>Target:</strong> 1 Enemy</span>
    <span><strong>Speed:</strong> Fast</span>
  </div>
  <p class="card-block__heading">Effect</p>
  <p>Can only be used if this character has Gear 2nd.</p>
  <p>Deal Power / (X + 1) damage (X + 1) times. Inflict X <a class="cc-status-link" data-status="status-frail" href="#">Frail</a>. Gain X <a class="cc-status-link" data-status="status-weak" href="#">Weak</a> and +X Weak Count. Gain X <a class="cc-status-link" data-status="status-vulnerable" href="#">Vulnerable</a> and +X Vulnerable Count.</p>
  <p>If this character has Gear 3rd: This Ultimate becomes Gomu Gomu no Gigant Rifle.</p>
</div>

### Ultimate-2: Gomu Gomu no Gigant Rifle
<div class="card-block">
  <div class="card-block__meta">
    <span><strong>Cost:</strong> 30 Ultimate Meter</span>
    <span><strong>Power:</strong> 36-54</span>
    <span><strong>Type:</strong> Ultimate, Attack, Physical, Melee, Splash, Blunt, Debuff</span>
    <span><strong>Target:</strong> 1 Enemy</span>
    <span><strong>Speed:</strong> Slow</span>
  </div>
  <p class="card-block__heading">Effect</p>
  <p>Can only be used if this character has Gear 3rd.</p>
  <p>Deal Power damage.</p>
  <p>If this character has Gear 2nd: This Ultimate becomes Gomu Gomu no Jet Gatling.</p>
</div>
