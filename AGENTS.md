...
This repo is the Chronicle Clash MkDocs Material rules reference with a documentation-only static site.
...
- Keyword/status/card type/term/role link markup uses the shared `cc-*-link` classes so `docs/javascripts/guide.js` can rewrite links.
...
- Chronicle Clash is documentation-only; do not add game client, relay server, or gameplay pages unless explicitly requested.
